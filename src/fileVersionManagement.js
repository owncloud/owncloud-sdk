...
class FilesVersions {
  ...
  listVersions (fileId) {
    ...
    return this.davClient.propFind(this.helpers._buildFullWebDAVPath(path), [], 1, {
  ...
  restoreFileVersion (fileId, versionId, targetPath) {
    ...
    return this.davClient.request('COPY', this.helpers._buildFullWebDAVPath(source), {
      Authorization: this.helpers.getAuthorization(),
      Destination: this.helpers._buildFullWebDAVURL(target)
  ...
  getFileVersionUrl (fileId, versionId) {
    const source = '/meta/' + fileId + '/v/' + versionId
    return this.helpers._buildFullWebDAVURL(source)
  }
}
